class OffsetsController < ApplicationController
  ...
  def manual_create
    if simple_captcha_valid?
      ...
      @offset = Offset.create(:purchased => 'TRUE',:user_id=>'0',:title=>params[:offset][:title],:cost=>params[:offset][:cost],:pounds=>pounds,:email => params[:user_email],:zipcode=>params[:offset][:zipcode])
      @stat = Stat.all.first
